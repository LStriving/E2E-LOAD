...
class Hdd(torch.utils.data.Dataset):
    """
    ...
    def __init__(self, cfg, mode, num_retries=100):
        ...
        logger.info("Constructing HDD {}...".format(mode))
    ...
    def _construct_loader(self):
        ...
        logger.info(
            "Constructing HDD dataloader (size: {}).".format(
                len(self._path_to_videos_work),
            )
        )  
        assert (
            len(self._path_to_videos_work) > 0
        ), "Failed to load HDD dataset"
    ...
    def __getitem__(self, index):
        ...
        if self.cfg.MODEL.LONG_MEMORY_ENABLE:
            ...
            patch_size = max(self.cfg.MODEL.CHUNK_SIZE // self.cfg.MODEL.CHUNK_SAMPLE_RATE,1)
        ...
        if self.cfg.MODEL.LONG_MEMORY_ENABLE:
            if self.cfg.DATA.ZERO_MASK:
                long_key_padding_mask_ = np.repeat(long_key_padding_mask, 1)
            ...
            # work_frames: torch.Size([3, 96, 224, 224])
            # long_frames: torch.Size([3, 24, 224, 224])
            # long_key_padding_mask: (8)
            # labels: (32, 22) = 96帧 /3 stride; 
        else:
