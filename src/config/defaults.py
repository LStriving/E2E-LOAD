...
# Evaluation Start Epochs
_C.TRAIN.EVAL_START_EPOCH = 1
...
# TRAIN, Long histroy sampler?

# _C.TRAIN.SEQUENCE_SAMPLER = 'uniform' # ‘interpolation’
_C.TRAIN.SEQUENCE_SAMPLER = "interpolation"

_C.TRAIN.SEQUENCE_PADDING = "zero"  # 'repeated'

_C.TRAIN.PRINT_VERPOSE = True
...
## Custom defined data augmentation strategies

_C.AUG.TEMPORAL = CfgNode()
# Temporal 时间抖动; 
_C.AUG.TEMPORAL.JITTER = CfgNode()
_C.AUG.TEMPORAL.JITTER.ENABLE = False # 是否使能; 默认为关闭状态
_C.AUG.TEMPORAL.JITTER.RATE = 0.1 # 发生的概率; 
_C.AUG.TEMPORAL.JITTER.MAX_EXTEND = 0.2 # 按照视频长度的百分比来做这件事; 
# _C.AUG.TEMPORAL.JITTER.SAMPLE_TYPE = 'uniform' 
_C.AUG.TEMPORAL.JITTER.SAMPLE_TYPE = 'random'
# 在时间轴上对数据进行平滑? 

_C.AUG.TEMPORAL.HISTORY = CfgNode() 

_C.AUG.TEMPORAL.HISTORY.ENABLE = False

_C.AUG.TEMPORAL.HISTORY.RANGE = 32


_C.AUG.TEMPORAL.SMOOTH = CfgNode() 

_C.AUG.TEMPORAL.SMOOTH.ENABLE = False

_C.AUG.TEMPORAL.SMOOTH.RATE = 0.1

_C.AUG.TEMPORAL.SMOOTH.MAX_ROLL = 3 # 几帧; 

_C.AUG.TEMPORAL.CUTMIX = CfgNode() 


_C.AUG.TEMPORAL.FRAME_START = CfgNode() # 整个视频的起始帧, 在chunk角度引入一定的随机性; 


_C.AUG.CURRENT = CfgNode()
    
_C.AUG.CURRENT.FPS_JITTER = CfgNode()
      
_C.AUG.CURRENT.FPS_JITTER.ENABLE = True

_C.AUG.CURRENT.SHUFFLE = CfgNode()

_C.AUG.CURRENT.SHUFFLE.ENABLE = True

_C.AUG.CURRENT.SHUFFLE.WIN_SIZE = 4

_C.AUG.CURRENT.SHUFFLE.RATE = 0.1

_C.AUG.CURRENT.MIX = CfgNode()
      
_C.AUG.CURRENT.MIX.ENABLE = True

_C.AUG.CURRENT.MIX.ALPHA = 0.8 #

_C.AUG.CURRENT.MIX.PROB = 0.5 #
...
# prediction results.
_C.TEST.NUM_ENSEMBLE_VIEWS = 1  # 因为没有在时间区域的采样, 所以这个ENSEMBLE参数应该设置为1;

# temporal crop shift range, enhance multi-view testing via expand context;
_C.TEST.CONTEXT_APPEND = 0.0  # 代表双帧?;
...
_C.TEST.SEQUENCE_PADDING = "zero"  # 'repeat'

_C.TEST.PRINT_VERPOSE = True
...
# _C.MODEL.DETACH_FINAL_FC = True

# If True, frozen batch norm stats during training.
_C.MODEL.FROZEN_BN = False
# _C.MODEL.FROZEN_BN = True

# If True, AllReduce gradients are compressed to fp16
_C.MODEL.FP16_ALLREDUCE = False

# Add the Setting for OAD;

_C.MODEL.MODE = "TRAIN"

_C.MODEL.LONG_MEMORY_SECONDS = 0.0

_C.MODEL.LONG_MEMORY_SAMPLE_RATE = 1

_C.MODEL.LONG_MEMORY_ENABLE = False  # 是否允许History, 从数据载入和模型建模层面都应该有所对应;

_C.MODEL.LONG_MEMORY_AUG_ENABLE = False  # 仅在有些Long不能使用数据增强的地方开始这个标志位;

_C.MODEL.WORK_MEMORY_SECONDS = 0.25

_C.MODEL.WORK_MEMORY_SAMPLE_RATE = 1

_C.MODEL.CHUNK_SAMPLE_RATE = 2

_C.MODEL.CASUAL_MASK_ENABLE = True

# process
_C.MODEL.MULTI_LABEL_NORM = False

_C.MODEL.MULTI_LABEL_SMOOTH = False

_C.MODEL.SMOOTH_VALUE = 0.1 
...
_C.MVIT.POOL_KVQ_KERNEL = None  # 统一各层的Kernel尺寸;
...
# the spatial branch of the MViT model;
_C.MVIT.SPATIAL = CfgNode()

_C.MVIT.SPATIAL.DEPTH = 10

_C.MVIT.SPATIAL.PATCH_DIMS_WORK = []

_C.MVIT.SPATIAL.PATCH_DIMS_LONG = []

_C.MVIT.SPATIAL.DIM_MUL = [[1, 2.0], [3, 2.0]]

_C.MVIT.SPATIAL.HEAD_MUL = [[1, 2.0], [3, 2.0]]

_C.MVIT.SPATIAL.POOL_Q_STRIDE = [
    [0, 1, 1],
    [1, 2, 2],
    [2, 1, 1],
    [3, 2, 2],
...
_C.MVIT.SPATIAL.POOL_KVQ_KERNEL = [3, 3]

_C.MVIT.SPATIAL.POOL_KV_STRIDE_ADAPTIVE = [8, 8]

_C.MVIT.SPATIAL.AGGREGATION = CfgNode()

_C.MVIT.SPATIAL.AGGREGATION.WORK_ENABLE = False

_C.MVIT.SPATIAL.AGGREGATION.LONG_ENABLE = False

_C.MVIT.SPATIAL.AGGREGATION.TYPE = "cls_token"


# the temporal branch of the MViT model;
_C.MVIT.TEMPORAL = CfgNode()

_C.MVIT.TEMPORAL.DEPTH = 6

_C.MVIT.TEMPORAL.DIM_MUL = [[4, 2.0]]

_C.MVIT.TEMPORAL.HEAD_MUL = [[4, 2.0]]

_C.MVIT.TEMPORAL.POOL_Q_STRIDE = [
    [0, 1, 1, 1],
    [1, 1, 1, 1],
    [2, 1, 1, 1],
    [3, 1, 1, 1],
...
# [[0, 1, 1, 1], [1, 1, 2, 2], [2, 1, 1, 1], [3, 1, 2, 2], [4, 1, 1, 1], [5, 1, 1, 1], [6, 1, 1, 1], [7, 1, 1, 1], [8, 1, 1, 1], [9, 1, 1, 1], [10, 1, 1, 1], [11, 1, 1, 1], [12, 1, 1, 1], [13, 1, 1, 1], [14, 1, 2, 2], [15, 1, 1, 1]]

_C.MVIT.TEMPORAL.POOL_KVQ_KERNEL = [3, 3, 3]

_C.MVIT.TEMPORAL.PATCH_DIMS = [] # 输入的尺寸; 

_C.MVIT.TEMPORAL.EMBED_DIM = 0

_C.MVIT.TEMPORAL.OUTPUT_DIM = 0

_C.MVIT.TEMPORAL.NUM_HEADS = (
    None  # number of the heads herint from the spatial dimension;
)

# setting of the position embeddings;

_C.MVIT.TEMPORAL.REL_POS_SPATIAL = True

_C.MVIT.TEMPORAL.REL_POS_TEMPORAL = True

_C.MVIT.TEMPORAL.USE_FIXED_SINCOS_POS = False

_C.MVIT.TEMPORAL.USE_ABS_POS = False

_C.MVIT.TEMPORAL.SEP_POS_EMBED = False

_C.MVIT.TEMPORAL.POOL_KV_STRIDE_ADAPTIVE = None

_C.MVIT.COMPRESSOR = CfgNode()

_C.MVIT.COMPRESSOR.DEPTH = 6

_C.MVIT.COMPRESSOR.DETACH = True

_C.MVIT.COMPRESSOR.PATCH_DIMS = [[4, 2.0]]

_C.MVIT.COMPRESSOR.DIM_MUL = [[4, 2.0]]

_C.MVIT.COMPRESSOR.HEAD_MUL = [[4, 2.0]]

_C.MVIT.COMPRESSOR.POOL_Q_STRIDE = [
    [0, 1, 1, 1],
    [1, 1, 1, 1],
    [2, 1, 1, 1],
    [3, 1, 1, 1],
...
_C.MVIT.COMPRESSOR.START_LAYER = 13

_C.MVIT.COMPRESSOR.POOL_KVQ_KERNEL = [3, 3, 3]

_C.MVIT.COMPRESSOR.PATCH_DIMS = []

_C.MVIT.COMPRESSOR.EMBED_DIM = 0

_C.MVIT.COMPRESSOR.OUTPUT_DIM = 0

_C.MVIT.COMPRESSOR.NUM_HEADS = None

_C.MVIT.COMPRESSOR.POOL_KV_STRIDE_ADAPTIVE = None

_C.MVIT.COMPRESSOR.CLS_TOKEN = False # Disable the cls token; 
_C.MVIT.COMPRESSOR.REL_POS_SPATIAL = True
_C.MVIT.COMPRESSOR.REL_POS_TEMPORAL = True
## Fusion Module

_C.MVIT.FUSION = CfgNode()

_C.MVIT.FUSION.DEPTH = 1 

_C.MVIT.FUSION.DROPOUT_RATE = 0.1 

_C.MVIT.FUSION.INITIALIZE_LAYER = 15 

_C.MVIT.FUSION.QUERY_PATCH_DIMS = [] 

_C.MVIT.FUSION.KEYVALUE_PATCH_DIMS = [] 

_C.MVIT.FUSION.EARLY_FUSION_LAYERS = [] 

_C.MVIT.FUSION.EARLY_FUSION_TYPE = 'kv'

_C.MVIT.FUSION.INTERACTION = CfgNode() # Cross Attention to Merge two features; 

_C.MVIT.FUSION.INTERACTION.QUERY_TYPE = 'spatial_temporal'


_C.MVIT.FUSION.INTERACTION.SPERATE_NORM = False

_C.MVIT.FUSION.INTERACTION.CLS_INCLUDE = False

_C.MVIT.FUSION.INTERACTION.REL_POS_TEMPORAL = True

_C.MVIT.FUSION.INTERACTION.REL_POS_SPATIAL = True

_C.MVIT.FUSION.INTERACTION.DEPTH = 1

_C.MVIT.FUSION.INTERACTION.EMBED_DIM = [768]

_C.MVIT.FUSION.INTERACTION.LAYER_SCALE_INIT_VALUE = 0.

_C.MVIT.FUSION.INTERACTION.DIM_OUT = [768]

_C.MVIT.FUSION.INTERACTION.NUM_HEADS = [8]

_C.MVIT.FUSION.ENHANCE = CfgNode()

_C.MVIT.FUSION.ENHANCE.ENABLE = False

_C.MVIT.FUSION.ENHANCE.LAYER_SCALE_INIT_VALUE = 0.

_C.MVIT.FUSION.ENHANCE.REL_POS_TEMPORAL = True

_C.MVIT.FUSION.ENHANCE.REL_POS_SPATIAL = True
...
_C.DATA.PROCESS_FPS = None  # 针对视频实际做决策的FPS;
...
# cfg.DATA about the long term history augmentation;
# ---------------------------------------------------------------------------- #
# Optimizer options
# ---------------------------------------------------------------------------- #

_C.DATA.ZERO_MASK = False
...
# -----------------------------------------------------------------------------
_C.TENSORBOARD = CfgNode()  # Replace it with your video;
...
_C.DEMO.CACHE_INFERENCE=True 

_C.DEMO.ALL_TEST=False
...
def assert_and_infer_cfg(cfg):  # 对有依赖关系的配置文件, 进行关系验证或者进行参数的重新赋值;
    # BN assertions.
    ...
    # model assertions.
    assert cfg.MVIT.SPATIAL.DEPTH + cfg.MVIT.TEMPORAL.DEPTH == cfg.MVIT.DEPTH

    # load the basic infos for the dataset;

    with open(cfg.DATA.DATA_INFO, "r") as f:
        data_info = json.load(f)[cfg.DATA.PATH_PREFIX]
    ...
    ## add the data_info;
    cfg.MODEL.LONG_MEMORY_NUM_FRAMES = int(
        cfg.MODEL.LONG_MEMORY_SECONDS * cfg.DATA.TARGET_FPS
    )  #
    cfg.MODEL.WORK_MEMORY_NUM_FRAMES = int(
        cfg.MODEL.WORK_MEMORY_SECONDS * cfg.DATA.TARGET_FPS
    )

    cfg.MODEL.TOTAL_MEMORY_NUM_FRAMES = (
        cfg.MODEL.LONG_MEMORY_NUM_FRAMES + cfg.MODEL.WORK_MEMORY_NUM_FRAMES
    )

    assert cfg.DATA.TARGET_FPS % cfg.DATA.PROCESS_FPS == 0

    cfg.MODEL.CHUNK_SIZE = cfg.DATA.TARGET_FPS // cfg.DATA.PROCESS_FPS
    assert (
        cfg.MODEL.LONG_MEMORY_NUM_FRAMES // cfg.MODEL.CHUNK_SIZE
    ) % cfg.MODEL.LONG_MEMORY_SAMPLE_RATE == 0
    assert (
        cfg.MODEL.WORK_MEMORY_NUM_FRAMES // cfg.MODEL.CHUNK_SIZE
    ) % cfg.MODEL.WORK_MEMORY_SAMPLE_RATE == 0

    cfg.MODEL.LONG_MEMORY_NUM_SAMPLES = int(
        (cfg.MODEL.LONG_MEMORY_NUM_FRAMES // cfg.MODEL.CHUNK_SIZE)
        // cfg.MODEL.LONG_MEMORY_SAMPLE_RATE
    )
    cfg.MODEL.WORK_MEMORY_NUM_SAMPLES = int(
        (cfg.MODEL.WORK_MEMORY_NUM_FRAMES // cfg.MODEL.CHUNK_SIZE)
        // cfg.MODEL.WORK_MEMORY_SAMPLE_RATE
    )

    cfg.MODEL.TOTAL_MEMORY_NUM_SAMPLES = (
        cfg.MODEL.LONG_MEMORY_NUM_SAMPLES + cfg.MODEL.WORK_MEMORY_NUM_SAMPLES
    )

    ## Alignment the model settings
    assert cfg.MVIT.TEMPORAL.DEPTH + cfg.MVIT.SPATIAL.DEPTH == cfg.MVIT.DEPTH
    return cfg
