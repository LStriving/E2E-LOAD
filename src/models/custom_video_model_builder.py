...
class STMViT(nn.Module):
    def __init__(self, cfg):
        ...
        # Prepare input.
        spatial_size = cfg.DATA.TRAIN_CROP_SIZE  # 224
        in_chans = cfg.DATA.INPUT_CHANNEL_NUM[0]  # 3

        temporal_size_long = cfg.MODEL.LONG_MEMORY_NUM_SAMPLES  
        temporal_size_work = cfg.MODEL.WORK_MEMORY_NUM_SAMPLES   
        ...
        self.T_long = (
            cfg.MODEL.LONG_MEMORY_NUM_SAMPLES // self.patch_stride[0]
        )  
        self.T_work = (
            cfg.MODEL.WORK_MEMORY_NUM_SAMPLES // self.patch_stride[0]
        )  
        ...
        self.cls_embed_on = cfg.MVIT.CLS_EMBED_ON  # Here is opened;
        self.use_mean_pooling = (
            cfg.MVIT.USE_MEAN_POOLING
        )  

        # Params for positional embedding
        self.use_abs_pos = cfg.MVIT.USE_ABS_POS 
        self.use_fixed_sincos_pos = (
            cfg.MVIT.USE_FIXED_SINCOS_POS
        ) 
        self.sep_pos_embed = cfg.MVIT.SEP_POS_EMBED  # False
        self.rel_pos_spatial = cfg.MVIT.REL_POS_SPATIAL  # True  
        self.rel_pos_temporal = cfg.MVIT.REL_POS_TEMPORAL  # True 
        ...
        dim_mul, head_mul = torch.ones(depth + 1), torch.ones(
            depth + 1
        )  

        for i in range(len(cfg.MVIT.DIM_MUL)):
            dim_mul[cfg.MVIT.DIM_MUL[i][0]] = cfg.MVIT.DIM_MUL[i][
                1
            ]   
        for i in range(len(cfg.MVIT.HEAD_MUL)):
            head_mul[cfg.MVIT.HEAD_MUL[i][0]] = cfg.MVIT.HEAD_MUL[i][
                1
            ] 
    ...
    def forward(self, x, bboxes=None, return_attn=False):
        x = x[0]  # 4 * 3 * 16 * 224 * 224 -> bsz, channels, num frames

        x, bcthw = self.patch_embed(x)  # ([2, 25088, 96])
        bcthw = list(bcthw)  # [2, 96, 8, 56, 56]
        ...
        if self.use_mean_pooling: 
            ...
            x = self.norm(x)
            x = x.mean(1)  # mean on the all class
