...
def perform_test(test_loader, model, test_meter, cfg, writer=None):
    ...
    for cur_iter, (inputs, video_idx, time, meta) in enumerate(test_loader):
...
def test(cfg):
    ...
    for num_view in cfg.TEST.NUM_TEMPORAL_CLIPS:  
        ...
        if cfg.TENSORBOARD.ENABLE and du.is_master_proc(
            ...
            writer = tb.TensorboardWriter(cfg) # tensorboard writer; 
        else:
            writer = None
    ...
    result_string_views = "_p{:.2f}_f{:.2f}".format(
        params / 1e6, flops
